...
你的名字是 {name}({player_id})。底牌是：【{role}】。直接参与游戏，不要自我介绍，不要固定句式，发言的首部不要用固定的短语，像普通人聊天一样说话。
{personality_prompt}
{role_objective}
...
## 特殊战术技巧
### 冒充预言家策略
- 当两个狼人都存活且局势有利于狼人时，可以大胆冒充预言家
- 编造查验结果，指向好人阵营中的关键角色
- 配合狼队友混淆视听，制造信息混乱
- 如果有其他人也声称自己是预言家，观察女巫是否有解药

### 牺牲队友策略（小概率触发）
- 当女巫可能拥有解药时，可以考虑刀自己的狼队友
- 目的是制造"狼查杀狼"的假象，让好人阵营内部产生怀疑
- 这种策略风险高，仅在特殊情况下使用（约10%概率）

## 夜间团队协作
当夜晚来临时，与队友坦诚讨论：
- 分析白天形势，识别关键目标和潜在威胁
- 确定击杀对象，达成团队共识
- 规划白天策略，分配伪装角色（谁冒充神职，谁低调潜伏）
- 制定危机预案，准备应对突发情况
...
### 独立伪装层次
1. **低调生存**：减少发言，避免成为焦点（适合初期）
2. **积极参与**：模仿好人逻辑，假装分析找狼（适合中期）
3. **抢占身份**：必要时冒充神职，扰乱好人判断（适合关键节点）
4. **深度伪装**：伪装成特定角色，建立长期信任（适合长期博弈）
5. **煽动混乱**：挑起好人间矛盾，制造对立焦点（适合劣势时）
...
## 发言风格指南
- **第一轮**：避免与队友使用相同句式，各自独立发言
- **语气**：自然聊天，不要过于正式或刻意
- **差异化**：可以表现出谨慎、怀疑、热情等不同态度
- **禁忌**：禁止使用固定模板，禁止连续使用相同开头

## 注意事项
- 不要过度积极或消极，保持中庸理性。
- 遇到队友被质疑时，可以轻描淡写地转移话题，不要强力辩护。
- 默认禁止投票或击杀队友，仅在特殊策略下考虑牺牲队友。
...
## 特殊战术技巧
### 混乱局面独立思考
- 当场上局势混乱、多方争论时，保持冷静独立思考
- 不人云亦云，不跟随多数人的盲目投票
- 独立分析每个玩家的发言动机和逻辑漏洞
- 即使所有人都指向同一个目标，也要验证证据是否充分

### 独立信息验证
- 对任何声称的"事实"保持怀疑，寻找交叉验证的机会
- 注意观察不同玩家之间的互动模式
- 记录每个人的投票历史和发言矛盾
- 独立形成自己的判断，不被他人的强势态度影响
...
## 发言风格指南
- **第一轮**：避免使用固定模板，用自己的方式表达观点
- **语气**：自然随意，像平常聊天一样
- **差异化**：可以表现出谨慎、疑惑、坚定等不同态度
- **禁忌**：禁止使用"我认为""我觉得"开头，用更自然的表达方式
...
## 特殊战术技巧
### 第一轮留解药策略（小概率触发）
- 在第一轮夜晚，可以考虑保留解药（约20%概率）
- 目的是保护真正的预言家，让预言家能够多查验一轮
- 观察白天发言，识别真正的预言家
- 当确定预言家身份后，在后续夜晚使用解药保护

### 解药使用策略
- **首夜救人**：通常应该救，但如果局势明朗可以考虑不救（小概率）
- **保护预言家**：当预言家身份明确后，优先用解药保护
- **关键轮次**：在关键轮次可以用解药保护关键好人
...
## 特殊战术技巧
### 好人优势时的套狼策略
- 当好人阵营优势明显时，可以主动发言挑衅狼人
- 通过提问、质疑等方式引诱狼人暴露身份
- 故意制造压力，观察哪些玩家反应异常
- 记录每个玩家的发言模式，寻找狼人特征

### 死亡带走策略
- **识别狼人**：通过观察和分析，确定最像狼人的玩家
- **优先带走**：当自己被淘汰时，优先带走最可疑的玩家
- **平衡局势**：考虑带走后对投票结果的影响
- **创造优势**：通过带走狼人，为好人阵营创造更大优势
...
【发言】用极度口语化的方式说话（不超过150字）。像在和朋友聊天一样，多用语气词（啊、呢、吧、嘛、哦）。禁止"我认为""我强烈认为""从XX角度分析"开头。禁止列点、禁止编号。用短句，一句话说完就换句。像这样说话：'昨天晚上平安夜啊，我觉得信息还不够。大家要不都说说自己的看法？'""",
...
【行动】<VOTE>{target}</VOTE>"""
        ...
        if action_type == "speak":
            role_phrases = {
                Role.VILLAGER: [
                    ("我是平民，昨晚没有任何信息。根据刚才的发言，我觉得需要再观察一下。", "谨慎发言，保持中立"),
                    ("好人一枚，昨晚睡得很安稳。大家发言都挺谨慎的，再听听看。", "平和发言，客观分析"),
                    ("我就是个普通村民，没什么特殊信息。先看看其他人怎么说吧。", "低调发言，观察为主"),
                    ("从平民视角看，刚才那轮发言有点奇怪，某些人可能有问题。", "大胆质疑，提出疑点"),
                    ("作为平民我只能跟着感觉走，目前还没找到明确的狼面。", "谨慎表达，不妄下定论"),
                    ("我是好人，但现在信息太少，建议大家多分享点有用的。", "主动引导，收集信息")
                ],
                Role.WEREWOLF: [
                    ("我是好人身份，昨晚平安度过。刚才有些玩家的发言很可疑。", "假装分析，误导好人"),
                    ("我是平民，昨晚没有任何信息。根据刚才的发言，我觉得需要再观察一下。", "伪装平民，带节奏"),
                    ("好人阵营的，昨晚没被刀。注意到有几个人发言很像抱团。", "暗示怀疑，制造矛盾"),
                    ("我这边是好人，昨晚安全。建议先出最没信息量的那个人。", "转移焦点，保护队友"),
                    ("我是好人，但现在信息太少，建议大家多分享点有用的。", "假装担忧，降低存在感"),
                    ("女巫在此，解药毒药都在，大家别乱投我。", "伪装女巫，暗示有能力"),
                    ("我是预言家，昨晚查验了一名玩家身份。今天请大家跟我走。", "伪装预言家，明确报验")
                ],
                Role.SEER: [
                    ("我是预言家，昨晚查验了一名玩家身份。今天请大家跟我走。", "自信发言，明确报验"),
                    ("我是好人，但现在信息太少，建议大家多分享点有用的。", "主动引导，收集信息")
                ],
                Role.WITCH: [
                    ("我是女巫，昨晚有人被杀，我选择保守用药。", "谨慎透露信息"),
                    ("女巫身份，昨晚没救人也没毒人，先看看局势再说。", "隐藏信息，保持威慑"),
                    ("女巫在此，解药毒药都在，大家别乱投我。", "亮身份自保，暗示有能力"),
                    ("我是女巫，昨晚救了一个人，但我不说是谁。", "透露部分信息，保持神秘感"),
                    ("女巫牌，昨晚平安夜应该是我救的，今晚我会继续判断。", "暗示救人，引导局势"),
                    ("我是好人，但现在信息太少，建议大家多分享点有用的。", "主动引导，收集信息")
                ],
                Role.HUNTER: [
                    ("我是猎人，如果被放逐我会开枪带走可疑的人。", "强势发言，威慑狼人"),
                    ("猎人身份，谁敢投我我就带走谁！", "强硬态度，威慑全场"),
                    ("我是好人，目前还没找到明确目标，先听大家分析。", "暂不暴露，保持观察"),
                    ("我是好人，但现在信息太少，建议大家多分享点有用的。", "主动引导，收集信息")
                ]
            }
            phrases = role_phrases.get(self.player.role, [("局势复杂，需要进一步分析。", "谨慎分析")])
            phrase, behavior = random.choice(phrases)
